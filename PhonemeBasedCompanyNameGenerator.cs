...
namespace CompanyNameGenerator
{
    public class Phoneme
    {
        ...
        public static readonly int LongestGraphemeLength = EnglishPhonemes
            .SelectMany(ep => ep.Graphemes)
            .Select(g => g.Length)
            .Max();
        public static readonly Dictionary<string, Phoneme> GraphemesToEnglishPhonemes = new Dictionary<string, Phoneme>(
            EnglishPhonemes
                .SelectMany(ep => ep.Graphemes.Select(g => new KeyValuePair<string, Phoneme>(g, ep)))
        );
    ...
    public class PhonemeBasedCompanyNameGenerator
    {
        public static List<Phoneme> WordToPhonemes(Dictionary<string, Phoneme> graphemesToEnglishPhonemes, int longestPhonemeLength, string word)
        {
            Dbc.Precondition(longestPhonemeLength > 0);
            Dbc.Precondition(!word.Any(c => Char.IsWhiteSpace(c))); // TODO: experiment with types instead?

            (string, Phoneme)? GetLongestGraphemeAndPhoneme(string substring)
            {
                while (substring.Length > 0)
                {
                    if (graphemesToEnglishPhonemes.TryGetValue(substring, out var phoneme))
                    {
                        return (substring, phoneme);
                    }
                    else
                    {
            ...
            while (word.Length > 0)
            {
                var longestGraphemeAndPhoneme = GetLongestGraphemeAndPhoneme(word);
                if (longestGraphemeAndPhoneme != null)
                {
                    var (grapheme, phoneme) = longestGraphemeAndPhoneme.Value;
                    word = word.Substring(grapheme.Length);
                }
                else
                {
                    // If we couldn't find a phoneme for the remainder of the word,
                    // just return what we have. If we skip a letter we may be inaccurate.
                    break;
                }
            }
